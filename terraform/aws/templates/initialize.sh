...
ksql ${ksql_server_url} <<EOF
  ...
  CREATE STREAM SONG_STAGE_1 AS SELECT UCASE(NAME) AS SONG_KEY, NAME, AUTHOR FROM SONG_WRAPPER;
  CREATE STREAM SONG_STAGE_2 AS SELECT 'CURRENT_SONG' AS CURRENT_SONG, SONG_KEY, NAME, AUTHOR FROM SONG_STAGE_1 PARTITION BY CURRENT_SONG;
  CREATE TABLE SONG (CURRENT_SONG VARCHAR, SONG_KEY VARCHAR, NAME VARCHAR, AUTHOR VARCHAR) WITH (KAFKA_TOPIC='SONG_STAGE_2', VALUE_FORMAT='JSON', KEY='CURRENT_SONG');
